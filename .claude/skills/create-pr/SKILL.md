...
メッセージも同様に日本語(`Co-Authored-By:`等のトレーラーは除く)。コード
識別子の英語ルール(AGENTS.md参照)はPRやコミットメッセージの文章には
適用しない。
...
1. **作業ツリーを確認する。** `git status` でクリーンであること。未コミット
   ...
   しない)。あわせて、今のブランチが今回の作業のために切られたものかを
   AGENTS.mdのブランチ運用ルール(新規の独立した作業は新しいブランチを
   切る)に沿って確認する。
